...
export async function syncMatches(formData?: FormData) {
  ...
  if (json.status !== 'success') {
    return { error: 'Failed to fetch from CricAPI' }
  }
...
export async function syncPlayers(formData?: FormData) {
  ...
  if (json.status !== 'success') {
    return { error: 'Failed to fetch squads from CricAPI' }
  }
  ...
  for (const team of json.data) {
    for (const player of team.players) {
      // Generate a random credit value between 8.0 and 10.5
...
export async function calculateScores(formData: FormData) {
  const matchId = formData.get('matchId') as string
  if (!matchId) return { error: 'Match UUID required' }
  ...
  if(N > 0) {
    ...
    while (currentRank <= N) {
      let tieCount = 1;
      let score = userRankings[currentRank - 1].rawScore;
